...
void game_interface()
{
	...
	outtextxy(200,385,"8");
	outtextxy(350,385,"9");
	readimagefile("images.turn.gif",0,-300,1200,500);
...
void x_o(int place)
{
	...
	{
		readimagefile("image.x.jpg",l,t,r,b);
	}
	if(ch=='O')
	{
		a[place-1]=0;
...
void function_execution(int choice)
{
	...
	{
		cleardevice();
		readimagefile("images.controls_key.jpg",200,0,900,600);
		esc_theme(3);
	}
	if(choice==4)
	{
		...
		for(int i=200;i<=1300;i++)
		{
			...
			readimagefile("images.about1.jpg",0,800-i,700,1300-i);
	...
	if(choice==5)
	{
		...
		outtextxy(140,330,"1");
		outtextxy(440,330,"2");
		readimagefile("images.quit.gif",50,100,650,400);
		readimagefile("images.yes.gif",-100,200,500,500);
		readimagefile("images.no,gif",200,200,800,500);
		quit();
	}
}
...
int main()
{
	initwindow(700,600,"Tic Tac Tae",150,150);
	loading_game();
	menu();
	selection();
	getch();
